...
class User_StudentController extends App_Controller_Action
{
	...
	$form->addElement('hidden', 'ParentUniqueRef', array('value' => $this->_auth->UniqueRef));
		...
		$form->addElement('hidden', 'EmailC', array('value' => '123456'));
                ...
                if (true === $result->Status) {
                    ...
                    $json['result']   = 1;
                    $json['callback'] = 'formSuccess()';
                 //     Zend_Debug::dump('Request returned form webservice');
                    $this->_helper->flashMessenger(array('success' => $result->StatusMessage));
                    
                } else {
		//	Zend_Debug::dump('Request returned form webservice');
                    $json['callback'] = 'displayMessage("Error : ' . $result->StatusMessage . '", "error");';
		    $this->_helper->flashMessenger(array('success' => $result->StatusMessage));
            ...
            $this->_helper->json($json);
           // $this->_helper->flashMessenger(array('user Added' => 'Added Users'));
    ...
    public function editAction()
    {
        ...
        foreach (array('EmailC', 'PasswordC') as $element)
